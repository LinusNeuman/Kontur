...
namespace NeonShooter
{
    public class Grid
    {
        class PointMass
        {
            ...
            public void ApplyForce(Vector3 position, float invMass)
            {
                Position = position;
                InverseMass = invMass;
            }
        ...
        struct Spring
        {
            ...
            public void Update()
            {
                ...
                //these sprigs can only pull, not push
            ...
            for (int y = 1; y < height; y++)
            {
                for (int x = 1; x < width; x++)
                {
                    Vector2 left = new Vector2(), up = new Vector2(); Vector2 p = ToVec2(points[x, y].Position); if (x > 1)
                    {
                        ...
                        //if the grid is very straight here, draw a single straight line. otherwiaw, draw lines to our
                    ...
                    if (x > 1 && y > 1)
                    {
                        ...
                        spriteBatch.DrawLine(0.5f * (upLeft + up), 0.5f * (left + p), color, 1f); // vertical lines
                        spriteBatch.DrawLine(0.5f * (upLeft + left), 0.5f * (up + p), color, 1f); // horizontal lines
                    }
